[![Build Status](https://github.com/libbitcoin/libbitcoin-protocol/actions/workflows/ci.yml/badge.svg?branch=version3)](https://github.com/libbitcoin/libbitcoin-protocol/actions/workflows/ci.yml)

[![Coverage Status](https://coveralls.io/repos/libbitcoin/libbitcoin-protocol/badge.svg?branch=version3)](https://coveralls.io/r/libbitcoin/libbitcoin-protocol?branch=version3)

# Libbitcoin Protocol

*The Bitcoin Blockchain Query Protocol Library*
...
On Linux and macOS libbitcoin-protocol can be build using Autotools and cmake. This process is accomplished via the provided installation scripts `install.sh` or `install-cmake.sh`. These scripts provide a basis for installation of the dependencies of the project. Both support the use of the argument `--help` for further parameterization.

Installation of libbitcoin-protocol comes with requirements listed below.
...
Below is a breakdown of preparation required to install libbitcoin-protocol. Once an appropriate [compiler](http://en.wikipedia.org/wiki/Compiler) and [build system](http://wikipedia.org/wiki/GNU_build_system) are in place, proceed to [Install Script Execution](#installscript) below to perform installation.
...
The [autotools install script](https://github.com/libbitcoin/libbitcoin-protocol/blob/version3/install.sh) and [cmake install script](https://github.com/libbitcoin/libbitcoin-protocol/blob/version3/install-cmake.sh) are written so that the manual build steps for each dependency can be inferred by a developer.

You can run either install script from any directory on your system. By default this will build libbitcoin-protocol in a subdirectory named `build-libbitcoin-protocol` and install it to `/usr/local/`. The install script requires `sudo` only if you do not have access to the installation location, which you can change using the `--prefix` option on the installer command line.
